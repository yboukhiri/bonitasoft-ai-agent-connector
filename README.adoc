...
The project **aiAgentConnector** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/aiAgentConnector.git
cd aiAgentConnector/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `aiAgentConnector-1.0-SNAPSHOT.jar`
